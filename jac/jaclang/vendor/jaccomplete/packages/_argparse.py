...
# This file contains argparse introspection utilities used in the course of jaccomplete execution.
...
class IntrospectiveArgumentParser(ArgumentParser):
    ...
    except for the lines that contain the string "Added by jaccomplete".
    '''

    def _parse_known_args(self, arg_strings, namespace, intermixed=False, **kwargs):
        _num_consumed_args.clear()  # Added by jaccomplete
        self._jaccomplete_namespace = namespace
        self.active_actions: List[Action] = []  # Added by jaccomplete
        ...
        self._action_conflicts = action_conflicts  # Added by jaccomplete
        ...
        self._seen_non_default_actions = seen_non_default_actions  # Added by jaccomplete
            ...
            if argument_values is not SUPPRESS or isinstance(action, _SubParsersAction):
                try:
                    ...
                    if isinstance(action, _SubParsersAction):
                        subnamespace = action._name_parser_map[argument_values[0]]._jaccomplete_namespace
                        for key, value in vars(subnamespace).items():
                            setattr(namespace, key, value)
                    # End added by jaccomplete
                    raise
        ...
        def consume_optional(start_index):
            ...
            while True:
                ...
                # if successful, exit the loop
                    ...
                    self.active_actions = [action]  # Added by jaccomplete
                    _num_consumed_args[action] = 0  # Added by jaccomplete
        ...
        def consume_positionals(start_index):
            ...
            for action, arg_count in zip(positionals, arg_counts):  # Added by jaccomplete
                self.active_actions.append(action)  # Added by jaccomplete
            for action, arg_count in zip(positionals, arg_counts):
                args = arg_strings[start_index : start_index + arg_count]
                start_index += arg_count
                _num_consumed_args[action] = len(args)  # Added by jaccomplete
        ...
        if positionals:
            self.active_actions.append(positionals[0])  # Added by jaccomplete
