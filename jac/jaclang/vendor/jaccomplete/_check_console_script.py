...
and verifying it contains the JAC_ARGCOMPLETE_OK marker.
...
Intended to be invoked by jaccomplete's global completion function.
"""
...
def main():
    ...
    # Look for the jaccomplete marker in the script it imports.
    with open(find(module_name, return_package=True)) as f:
        head = f.read(1024)
    if "JAC_ARGCOMPLETE_OK" not in head:
