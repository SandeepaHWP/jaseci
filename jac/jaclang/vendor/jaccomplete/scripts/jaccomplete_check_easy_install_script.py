...
"""
This script is part of the Python jaccomplete package (https://github.com/kislyuk/jaccomplete).
It is used to check if an EASY-INSTALL-SCRIPT wrapper redirects to a script that contains the string
"JAC_ARGCOMPLETE_OK". If you have enabled global completion in jaccomplete, the completion hook will run it every
time you press <TAB> in your shell.

Usage:
    python-jaccomplete-check-easy-install-script <input executable file>
"""
        ...
        if line1.startswith("#") and ("py" in line1 or "Py" in line1):
            ...
            for line in lines:
                if line.startswith("# EASY-INSTALL-SCRIPT"):
                    ...
                    if "JAC_ARGCOMPLETE_OK" in pkg_resources.get_distribution(dist).get_metadata(
                        "scripts/" + script
                    ):
                        return 0
                    ...
                    with open(pkgutil.get_loader(module_name).get_filename()) as mod_fh:  # type: ignore
                        if "JAC_ARGCOMPLETE_OK" in mod_fh.read(1024):
                            return 0
