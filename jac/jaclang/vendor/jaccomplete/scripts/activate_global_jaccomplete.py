...
"""
Activate the generic bash-completion script or zsh completion autoload function for the jaccomplete module.
"""
...
def get_activator_dir():
    return os.path.join(os.path.abspath(os.path.dirname(jaccomplete.__file__)), "bash_completion.d")


def get_activator_path():
    return os.path.join(get_activator_dir(), "_python-jaccomplete")
    ...
    if os.path.exists(path):
        with open(path, 'r') as fh:
            if shellcode in fh.read():
                ...
                return
        print(f"jaccomplete needs to append to the file {path}. The following code will be appended:", file=sys.stderr)
...
def main():
    ...
    if args.dest:
    ...
    elif site.ENABLE_USER_SITE and site.USER_SITE and site.USER_SITE in jaccomplete.__file__:
        print(
        ...
        destinations.append(f"{get_zsh_system_dir()}/_python-jaccomplete")
        destinations.append(f"{get_bash_system_dir()}/python-jaccomplete")
